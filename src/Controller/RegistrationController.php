...
class RegistrationController extends Controller
{
    ...
    public function register(Request $request, UserPasswordEncoderInterface $passwordEncoder): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $user->setUsername(
                    strtolower($user->getFirstname()[0]).strtolower($user->getName()).$user->getInscriptionYear()
            );
            ...
            $user->setEmail(
                    strtolower($user->getFirstname()).".".strtolower($user->getName()).$user->getInscriptionYear(
                    ).'@campus-eni.fr'
            );
...
//                    $form->get('plainPassword')->getData()
            ...
            if (
                    ((strcasecmp($user->getName(), 'Laz') == 0) && (strcasecmp(
                                            $user->getFirstname(),
                                            'Baptiste'
                                    ) == 0)) ||
